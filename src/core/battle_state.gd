...
## The enemy captain can be attacked when their line is thin, their reserves
## are spent, or a card forced a window. Routed enemies are off the field, so
## breaking morale opens the window just like killing does.
func enemy_captain_targetable() -> bool:
	...
	if captain_forced_exposed or duel_active:
		return true
	return enemy_field.size() <= CAPTAIN_EXPOSED_FIELD_SIZE or enemy_reserve.is_empty()
