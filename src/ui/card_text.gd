...
## How this card is played, for the card's own footnote: the targeted cards
## name a man, the rest land anywhere.
...
[b]Each turn[/b]: you open holding the 5 cards dealt at the end of your last turn (the boarding deals the first hand) plus whatever Retained cards (Trade Places, Drag Him Back!) you are keeping for their moment. THE OPENING comes first, one forced choice before anything may be played: send a man across the rail FREE into a slot you pick, SWAP two of your own men into each other's places (on deck or across the rail), or take the income — +1 momentum AND +1 card on top of the turn's own +1. That income is exactly what a free crossing or a free swap costs you. Then play cards — most also move one of your men, and that move is mandatory and its DIRECTION IS PRINTED ON THE CARD: port, starboard, press forward, give ground, or close on the nearest enemy. The board lights up every man who could take that step and you pick which one, never whether and never which way — and a card whose step nobody can take cannot be played at all. Then both formations fight — axes first (they chew 2 block per point of damage), then the rest by speed. Every man follows his rhythm: berserkers build to a doubled HEAVY blow, archers AIM before both arrows fly (the mark comes away suppressed, a third weaker), shieldmen raise the wall before they swing. Armor is guard: it comes back up as block at each side's turn. Then your hand cycles: unplayed cards are discarded (Retained ones stay) and you draw 5 fresh — always 5, a Retained card costs no draw; only a full hand of 7 stops the deal.
...
[b]The rail[/b]: enemies reinforce 2 per turn into their front gaps. Your own men come over with the turn's opening, free — that is the only crossing there is; Trade Places is a second swap in the same turn. The reserve itself never fights and is never hit. Your captain and his prowman are alternates: one of them always holds the field, and only Trade Places trades them, so the waiting one shows dimmed on your rail. Retreat is always on the table — a live crew beats a dead legend."""
