...
[b]Each turn[/b]: your hand is discarded and redrawn to 5 — except Retained cards (Reinforce, Trade Places, Drag Him Back!), which wait in hand for their moment. Then THE OPENING, one forced choice before anything may be played: send a man across the rail FREE into a slot you pick, SNAP two of your own men into each other's places (on deck or across the rail), or take the income — +1 momentum AND +1 card on top of the turn's own +1. That income is exactly what a free crossing or a free snap costs you. Then play cards — most also move one of your men, and that move is mandatory and its DIRECTION IS PRINTED ON THE CARD: port, starboard, press forward, give ground, or close on the nearest enemy. The board lights up every man who could take that step and you pick which one, never whether and never which way — and a card whose step nobody can take cannot be played at all. Then both formations fight — axes first (they chew 2 block per point of damage), then the rest by speed. Every man follows his rhythm: berserkers build to a doubled HEAVY blow, archers AIM before both arrows fly (the mark comes away suppressed, a third weaker), shieldmen raise the wall before they swing. Armor is guard: it comes back up as block at each side's turn.
...
[b]The rail[/b]: enemies reinforce 2 per turn into their front gaps. Your own men come over with the turn's opening, free — and the Reinforce card is a second crossing in the same turn, Trade Places a second snap. The reserve itself never fights and is never hit. Your captain and his prowman are alternates: one of them always holds the field, and only Trade Places trades them, so the waiting one shows dimmed on your rail. Retreat is always on the table — a live crew beats a dead legend."""
